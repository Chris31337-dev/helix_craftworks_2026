...
type EcwidWindow = typeof globalThis & {
  ...
  return (
    <header className="flex items-center justify-between">
      ...
      <nav className="hidden items-center gap-6 text-sm text-steel sm:flex">
        ...
        <a
          href="/#contact"
          className="rounded-full bg-redwood px-4 py-2 text-canvas shadow-card transition hover:-translate-y-0.5"
        >
          Request a consult
        </a>
      </nav>
...
function loadEcwid(): void {
  const initBrowser = () => {
    ...
    const w = globalThis as EcwidWindow;
      ...
      <main className="mt-16 flex flex-col gap-8 sm:gap-10">
        <section className="space-y-4 rounded-2xl border border-canvas/10 bg-canvas/5 p-6 shadow-card">
          ...
          <p className="max-w-2xl text-sm text-steel">
            Apparel and small-batch goods from {BRAND_MARK}. Browse and purchase directly through
            the catalog below.
          </p>
        </section>
